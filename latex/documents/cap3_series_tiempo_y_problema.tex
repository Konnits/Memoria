...
\begin{itemize}
    \item A nivel de construcción del dataset, se define un \textit{offset de predicción} $o$ que indica cuántos índices después del \emph{anchor} se tomará el target en la secuencia original.
    \item A nivel del modelo, además, se utilizan los timestamps reales $t_i$ para construir tiempos relativos respecto del primer timestamp válido de la ventana. Así, tanto las observaciones históricas como el instante objetivo se expresan en una escala común, la distancia temporal entre la última observación de la historia y el objetivo queda disponible como diferencia entre sus tiempos relativos, no como un cambio del origen temporal.
\end{itemize}
...
La familia \CTTransformer\ recibirá como entrada una representación construida a partir de la historia y del timestamp objetivo. En la arquitectura base esto se realiza mediante la adición de tokens objetivo a la misma secuencia de entrada, en la variante encoder--decoder, esos tokens se procesan en el decoder y consultan la historia codificada por el encoder. En ambos casos, el modelo deberá producir una estimación $\widehat{\mathbf{y}}^{(\text{target})}$ de la salida verdadera. Los detalles de esta representación se desarrollan en el capítulo dedicado al modelo Transformer.
