...
En este capítulo se formaliza el problema que aborda esta memoria y se introducen los conceptos básicos necesarios para describirlo con precisión. El objetivo es establecer un marco formal que permita entender qué se espera de \CTTransformer\ y cómo se construyen los ejemplos supervisados a partir de datos temporales con marcas de tiempo no equiespaciadas.
...
De manera general, una \textit{serie de tiempo} es una secuencia de observaciones ordenadas en el tiempo. En su forma más simple (caso univariado) se puede escribir como:
\begin{equation}
...
\begin{equation}
    t_1, t_2, \dots, t_T
\end{equation}
ya no están separados por un intervalo constante, y la información contenida en las diferencias $t_{i+1} - t_i$ pasa a ser relevante. A este tipo de datos nos referiremos como \textit{series de tiempo no equiespaciadas} o \textit{con timestamps irregulares}.
...
Estas características motivan la necesidad de modelos capaces de incorporar explícitamente la información temporal continua, lo que se abordará en capítulos posteriores mediante mecanismos de \textit{encoding temporal continuo} dentro de la arquitectura Transformer.
...
\begin{itemize}
    \item \textbf{Datos reales procesados del benchmark legado}, organizados como cinco series temporales independientes. En ese protocolo las ventanas y offsets se definen por cantidad de eventos.
    \item \textbf{Datos sintéticos del benchmark físico}, organizados en ocho presets, cada uno con una unidad univariada y otra multivariada asincrónica. Cada unidad conserva por separado observaciones irregulares con ruido y una trayectoria limpia densa usada como fuente de targets.
\end{itemize}

\hfill

Ambos escenarios comparten una separación temporal de entrenamiento, validación y prueba, pero no comparten el mismo estimando. El protocolo legado pregunta por desempeño a offsets ordinales sobre datos reales; el protocolo físico pregunta si el modelo identifica duraciones y gaps reales en procesos sintéticos controlados. Sus scores no se comparan directamente.

\hfill

La separación es metodológica. Los datos reales aportan una prueba de factibilidad y relevancia aplicada, pero no entregan una trayectoria contrafactual limpia en instantes sin observación. Los datos sintéticos permiten consultar esa trayectoria en tiempos arbitrarios, variar controladamente el patrón de muestreo y ejecutar corruptelas temporales emparejadas. Por ello son el dominio apropiado para atribuir efectos al uso del tiempo físico, aunque no establecen validez externa sobre sistemas reales.
...
Por motivos computacionales y de regularización, se trabaja con una \textit{historia truncada} de longitud acotada. En esta memoria se utiliza una ventana deslizante que, para cada ejemplo, considera sólo las últimas $h$ observaciones antes de $t^\star$:
\begin{equation}
    \mathcal{H}^{(h)}_i
...
\begin{itemize}
    \item En el protocolo legado, un \textit{offset de predicción} $o$ indica cuántos eventos después del \emph{anchor} se toma el target.
    \item En el protocolo físico, el horizonte es una duración $\delta>0$ y la consulta se fija en $t^\star=t_0+\delta$, donde $t_0$ se toma de la grilla de la trayectoria latente limpia con una regla independiente de la llegada de eventos.
    \item En ambos casos los timestamps absolutos se recentran dentro de la ventana. En el protocolo físico esta resta se realiza en precisión \textttbreak{float64} y sólo los tiempos relativos se convierten a la precisión de la red.
\end{itemize}
...
\subsection{Contrato de tiempo físico e identificabilidad}

Para una duración histórica $D$ y un origen de forecast $t_0$, el contexto físico se define como
\begin{equation}
    \mathcal{H}_{D}(t_0)=\{(t_i,\mathbf{x}_i):t_0-D\leq t_i\leq t_0\}.
\end{equation}
Todos los ejemplos cubren así el mismo intervalo físico, aunque contengan distinta cantidad de eventos. Si la historia excede el presupuesto computacional, se submuestrea preservando cobertura temporal; una historia vacía se descarta y no se fabrica una observación cero.

Cada ejemplo contiene varias consultas $t_m^\star=t_0+\delta_m$. En entrenamiento los $\delta_m$ se muestrean log-uniformemente en $[0.25,8.0]$; en evaluación se usan $\{0.25,1.0,3.0,8.0\}$. Su orden se permuta reproduciblemente para impedir que el índice del token revele el horizonte. El target $\mathbf{y}(t_m^\star)$ se obtiene por interpolación lineal de la trayectoria sintética limpia y no del siguiente evento observado.

La información temporal es \emph{identificable} sólo si cambiarla manteniendo valores, consultas y targets emparejados puede alterar el desempeño de un modelo que la utiliza, y si controles sin tiempo físico no recuperan la misma señal mediante proxies. Por ello el protocolo compara tiempo real con timestamps igualados, gaps permutados, grilla regular, representación ordinal y vistas que conservan sólo historia o sólo queries. También contrasta persistencia, un control ordinal y un regresor con horizonte físico explícito antes de interpretar resultados neuronales.

\section{Construcción de ejemplos supervisados}

\subsection{Ventanas deslizantes y tamaño de la historia}

Sea $T$ el número total de observaciones disponibles. Dado un \textit{tamaño máximo de historia} $h_{\max}$ y un \textit{stride} $s \ge 1$, se generan ejemplos recorriendo la serie con una ventana deslizante. De manera simplificada, los índices \emph{anchor} se pueden escribir como:
\begin{equation}
    a \in \{h_{\max}, h_{\max}+s, h_{\max}+2s, \dots, a_{\max}\},
\end{equation}
...
\end{equation}
donde $h$ puede tomar valores entre una longitud mínima $h_{\min}$ y la longitud máxima $h_{\max}$:
\begin{equation}
...
\begin{itemize}
    \item \textbf{Historia fija}: $h_{\min} = h_{\max}$, por lo que todas las historias tienen la misma longitud.
    \item \textbf{Historia variable}: $h_{\min} < h_{\max}$, y para cada ejemplo se muestrea aleatoriamente una longitud $h \in [h_{\min}, h_{\max}]$. De este modo, el modelo se entrena para manejar ventanas de contexto de tamaño variable, algo relevante cuando la densidad temporal de los datos cambia en el tiempo.
\end{itemize}
...
La familia \CTTransformer\ recibe una representación construida a partir de la historia y de los timestamps objetivo. En la arquitectura base se añaden tokens objetivo a la secuencia; en encoder--decoder se procesan en el decoder; en \QueryCross\ son consultas independientes que nunca entran al encoder, y en \ContinuousBasis\ evalúan una función explícita del horizonte. Los detalles se desarrollan en el capítulo dedicado al modelo.
...
Sobre este marco se construirá, en los capítulos siguientes, la arquitectura \CTTransformer\ para manejar timestamps no equiespaciados y realizar predicciones a distintos horizontes, junto con los experimentos que evalúan su desempeño.
