...
\begin{itemize}
    \item La elección del paso de malla introduce un \textit{compromiso} entre resolución temporal y costo computacional.
    \item La imputación puede introducir \textit{sesgos} sistemáticos cuando la irregularidad está asociada a la dinámica del sistema (por ejemplo, muestreo más denso durante eventos anómalos).
...
\begin{itemize}
    \item Maneja \textit{directamente} muestreo irregular: basta con evaluar el kernel en los tiempos observados.
    \item Proporciona \textit{predicciones probabilísticas} (medias e intervalos de confianza) para cualquier tiempo futuro.
...
\subsection[Modelos en tiempo continuo: Neural ODE y variantes]{Modelos en tiempo continuo: Neural ODE, ODE-RNN y GRU–ODE–Bayes}

Un segundo grupo de trabajos adopta una perspectiva de \textit{tiempo continuo}, en la cual la dinámica latente de la serie se modela mediante ecuaciones diferenciales. En lugar de actualizar el estado oculto en pasos discretos fijos, estos modelos integran una EDO entre observaciones, lo que permite manejar de manera natural tiempos de muestreo irregulares.
...
Los modelos basados en \textit{ecuaciones diferenciales controladas} (CDE) generalizan la idea anterior modelando la trayectoria latente $z_t$ como solución de una ecuación
\[
    \mathrm{d} z_t = f_\theta(z_t)\, \mathrm{d} X_t,
\]
donde $X_t$ es una versión suavizada o interpolada de la serie de entrada, considerada como un \emph{camino control} en tiempo continuo. Las \textit{Neural CDE} parametrizan $f_\theta$ mediante una red neuronal y resuelven la ecuación con un solucionador numérico, de forma análoga a las Neural ODE, pero interpretando las entradas como controles que impulsan la dinámica \cite{kidger2020neural}.

\hfill

Esta formulación tiene varias ventajas para series irregulares:
\begin{itemize}
    \item La interpolación continua de las observaciones permite manejar \textit{cualquier calendario de muestreo}, siempre que se defina una manera razonable de rellenar los huecos entre puntos (por ejemplo, interpolación lineal o splines).
    \item El modelo es inherentemente \textit{invariante a reparametrizaciones} del tiempo si se utilizan interpolaciones apropiadas, lo que lo hace robusto a cambios de resolución temporal.
    \item Se puede demostrar que las Neural CDE son capaces de aproximar una clase amplia de funcionales sobre caminos, lo que las hace apropiadas para tareas tanto de \textit{forecasting} como de clasificación.
\end{itemize}
...
\begin{itemize}
    \item Las observaciones están en una malla \textit{equiespaciada}, el índice de tiempo se puede tratar como un entero $1,\dots,T$.
...
Una línea de trabajo más reciente se enfoca en \textit{representaciones continuas del tiempo} para integrarlas en arquitecturas tipo \Transformer. Entre las propuestas relevantes se encuentran:
\begin{itemize}
    ...
    \item Trabajos que estudian la relación entre \textit{positional encodings discretos y continuos}, mostrando que los codificadores sinusoidales clásicos pueden interpretarse como evaluaciones discretas de funciones definidas sobre tiempo continuo, y explorando extensiones que permiten evaluar el encoding en tiempos reales intermedios. Esto abre la puerta a reemplazar el índice entero por un \emph{timestamp} real normalizado.
\end{itemize}

Estas ideas son particularmente relevantes para series irregulares: si el \emph{positional encoding} puede evaluarse en cualquier tiempo, es posible definir un \textit{encoding temporal continuo} que reciba directamente los \emph{timestamps} reales, preservando la estructura del \Transformer\ original. Esta es precisamente la filosofía que se adopta en el modelo de esta memoria, descrito en los Capítulos~\ref{cap:transformers-series-tiempo} y~\ref{cap:modelo-no-equiespaciado}.

\subsection{Transformers sobre observaciones irregulares y modelos híbridos}

Más allá de los \emph{encodings} continuos, una línea muy relevante del estado del arte reciente consiste en construir \textit{tokens directamente a partir de observaciones irregulares}, evitando la regularización previa sobre una malla fija. En esta familia destacan, al menos, dos estrategias representativas.

La primera consiste en tratar la serie como un \textit{conjunto de eventos u observaciones}. Un ejemplo claro es \textit{STraTS} \cite{tipirneni2022strats}, que representa cada medición mediante una tripleta $(t,f,v)$ compuesta por tiempo, variable y valor. El modelo construye un embedding inicial sumando tres componentes: un embedding de variable, un embedding continuo del valor y un embedding continuo del tiempo. Estas representaciones se contextualizan con un encoder \Transformer\ y luego se fusionan mediante atención para producir un embedding global de la serie. Su contribución principal es demostrar que un esquema de atención puede operar directamente sobre datos clínicos dispersos e irregulares sin recurrir a imputación o agregación horaria agresiva. Además, incorpora un objetivo de auto-supervisión basado en \emph{forecasting}, pero dicho objetivo se utiliza como tarea auxiliar para mejorar las representaciones, la validación principal del trabajo corresponde a clasificación clínica. Por ello, STraTS es cercano a esta memoria en el uso explícito de tiempos reales, pero se diferencia en que no formula el pronóstico como consultas futuras mediante tokens objetivo ni como un esquema encoder--decoder.

La segunda estrategia consiste en definir mecanismos de atención que respeten explícitamente la estructura irregular en los ejes temporal y multivariable. En esta línea, \textit{CoFormer} \cite{wei2023coformer} modela cada muestra como un punto \emph{variate-time}: una medición asociada a una variable y a un instante continuo. Primero combina un embedding de medición con códigos de variable y tiempo, luego aplica de forma sucesiva atención \emph{intra-variate}, que conecta mediciones de la misma variable, y atención \emph{inter-variate}, que conecta mediciones de variables distintas cercanas en el tiempo. Finalmente agrega las representaciones por variable y después a nivel de observación. Frente a un encoder \Transformer\ completamente estándar, CoFormer introduce un sesgo inductivo más fuerte para la desalineación temporal y multivariable. Esta fortaleza depende precisamente de la existencia de varias variables asincrónicas, en escenarios univariados, la rama intervariable pierde parte de su función original.

Junto con estos modelos orientados a observaciones, siguen existiendo propuestas que integran nociones explícitas de \textit{tiempo continuo} en la trayectoria latente o en la representación de la secuencia:
\begin{itemize}
    \item Modelos que combinan \Transformers\ con \textit{Neural ODE} o \textit{Neural CDE}, utilizando la atención para resumir subsecuencias o para parametrizar el campo vectorial de la EDO/ECDE, y resolviendo luego la dinámica en tiempo continuo.
    \item Propuestas como el \textit{Rough Transformer}, que se apoyan en herramientas de \emph{rough path theory} y firmas de caminos (\emph{signatures}) para representar trayectorias irregulares, sobre las que se aplica atención. La idea central es mapear la serie irregular a un conjunto de características invariantes a reparametrizaciones del tiempo, que luego sirven como ``tokens'' de entrada al \Transformer\ \cite{moreno2024rough}.
\end{itemize}

En conjunto, estos trabajos muestran que el panorama actual ya no se limita a ``transformers regulares + imputación'', sino que incluye arquitecturas capaces de operar directamente sobre observaciones asincrónicas. Sin embargo, persiste un compromiso claro entre \textit{generalidad arquitectónica}, \textit{sesgo inductivo específico para la irregularidad} y \textit{simplicidad de implementación}. Precisamente ahí se sitúa la motivación de esta memoria: capturar la información temporal continua sin abandonar la estructura simple y reutilizable de un encoder \Transformer\ convencional.

\subsection{Agregación por \emph{patches} y baselines simples para series irregulares}

Muy recientemente han aparecido trabajos que replantean el \emph{forecasting} de series irregulares desde una perspectiva de \textit{agregación por bloques} (\emph{patch aggregation}). Una propuesta representativa introduce un módulo de agregación consciente del tiempo, que agrupa observaciones irregulares en \emph{patches} definidos sobre ventanas temporales y aplica operaciones de pooling o atenciones locales para producir una secuencia de tokens en una malla pseudo-regular. Esta secuencia agregada se procesa luego con redes relativamente simples (por ejemplo, MLP o bloques de atención ligeros) \cite{liu2025rethinking}.
...
\end{itemize}
puede competir o superar a arquitecturas mucho más complejas basadas en ODE/CDE o \Transformers\ especializados \cite{liu2025rethinking}. Esto sugiere que, en series irregulares, \textit{cómo se resumen los datos en el tiempo} puede ser tan importante como la arquitectura posterior.
...
Aunque el foco principal de esta memoria es el \emph{forecasting}, muchas de las técnicas discutidas se han desarrollado inicialmente para tareas de \textit{clasificación} sobre series irregulares, especialmente en el dominio de la salud. En esta sección se resume brevemente el estado del arte en clasificación, poniendo énfasis en su relación con los modelos ya descritos.
...
Buena parte de los modelos de deep learning revisados en la Sección~\ref{sec:estado-arte-deep} se introdujeron inicialmente para \textit{clasificación} más que para forecasting. Ejemplos importantes incluyen:
\begin{itemize}
...
En modelos que producen un embedding global de la serie, la adaptación entre clasificación y regresión puede reducirse en parte a cambiar la capa de salida y la función de pérdida. Sin embargo, el \textit{forecasting} multi-horizonte introduce una decisión adicional: cómo condicionar la predicción en los instantes futuros concretos. Esta distinción es relevante para esta memoria, porque \CTTransformer\ representa cada instante de predicción mediante un token objetivo con su timestamp, mientras que varios modelos de clasificación irregular, como STraTS y CoFormer en sus formulaciones originales, agregan primero la historia en una representación global.

\subsection{Transformers para clasificación de series irregulares}

El uso de \Transformers\ para clasificación con observaciones irregulares ha madurado especialmente en el dominio biomédico, aunque la literatura sigue estando relativamente \textit{fragmentada por tarea y dominio}. En términos generales, pueden distinguirse tres estrategias.
...
La evidencia reciente sugiere que los \Transformers\ sí pueden ser competitivos en clasificación irregular cuando la representación temporal y la tokenización respetan la asincronía de las observaciones. No obstante, la mayor parte de los resultados más sólidos siguen concentrados en \textit{clasificación clínica} y no necesariamente se trasladan de forma directa a problemas generales de \textit{forecasting} multivariante irregular. Esta separación entre literatura de clasificación y literatura de pronóstico es una de las razones por las que sigue siendo valioso estudiar arquitecturas simples y transferibles entre ambas tareas.

\section{Discusión y brechas de investigación}
\label{sec:brechas-estado-arte}

A partir de la revisión anterior, se pueden identificar varias brechas que motivan el diseño de \CTTransformer\ en esta memoria:
...
\begin{itemize}
    \item Diseñar un \textit{encoding temporal continuo} que reciba directamente \emph{timestamps} reales, inspirado en los posicional encodings sinusoidales clásicos, pero normalizado por una escala temporal configurable.
    \item Construir la tarea de forecasting mediante un \textit{token objetivo} que representa explícitamente el instante de predicción dentro de la misma secuencia que la historia, permitiendo reutilizar un encoder \Transformer\ estándar \cite{vaswani2017attention}.
    \item Explorar el entrenamiento con \textit{historias de longitud variable} y \textit{múltiples offsets de predicción} dentro de un mismo modelo, evaluando cómo estas decisiones de diseño afectan la capacidad de generalización en escenarios sintéticos y reales.
\end{itemize}

Los capítulos siguientes desarrollan en detalle esta propuesta: el Capítulo~\ref{cap:series-tiempo} formaliza el problema y la construcción de ejemplos supervisados, el Capítulo~\ref{cap:transformers-series-tiempo} presenta la arquitectura base de \Transformers\ para series de tiempo, y el Capítulo~\ref{cap:modelo-no-equiespaciado} describe \CTTransformer, junto con su implementación concreta.
