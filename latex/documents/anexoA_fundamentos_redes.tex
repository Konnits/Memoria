...
\begin{itemize}
    ...
    \begin{quote}
        \textbf{Nota:} Se utiliza una función de activación porque la combinación lineal de transformaciones lineales es, en sí misma, una transformación lineal; por lo tanto, la predicción estaría limitada a funciones objetivo lineales.
    \end{quote}
...
\[\hat{y}_1 \quad \hat{y}_2 \quad \hat{y}_{_{\text{out}}}\]
...
En esta memoria se utiliza Adam como optimizador por defecto para entrenar el modelo Transformer, tal como se especifica en los archivos de configuración de entrenamiento.

\section{Relación con el modelo Transformer de la memoria}
...
\begin{itemize}
    ...
    \item La red completa se entrena minimizando una función de pérdida (en este caso, típicamente MSE sobre las predicciones de la serie de tiempo) usando descenso de gradiente y un optimizador como Adam.
...
Por lo tanto, comprender el funcionamiento de neuronas, funciones de activación, retropropagación y Adam es suficiente para entender el entrenamiento de toda la arquitectura propuesta, aunque los detalles de la atención y los encodings temporales se desarrollen en capítulos específicos del cuerpo principal.
...
\begin{itemize}
    ...
    \item El algoritmo Adam como optimizador adaptativo ampliamente utilizado en la práctica.
\end{itemize}

Estos conceptos constituyen la base sobre la cual se construyen las arquitecturas específicas analizadas en la memoria, en particular el modelo Transformer adaptado a series de tiempo no equiespaciadas.
