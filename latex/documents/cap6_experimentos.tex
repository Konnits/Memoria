...
El runner ofrece los modos \textttbreak{preflight}, \textttbreak{dry-run}, \textttbreak{smoke}, \textttbreak{run} y \textttbreak{report}, además de \textttbreak{all} para ejecutar la secuencia completa. El modo \textttbreak{smoke} usa un presupuesto reducido para validar lectura de datos, forward, backward, checkpoints y reportes; sus métricas no constituyen un ranking. Sólo \textttbreak{run} completo, seguido de \textttbreak{report}, puede poblar las tablas finales del Capítulo~\ref{cap:resultados}. La salida v2 se guarda bajo \nolinkurl{experiments/thesis_physical_benchmark_v2/}; los artefactos parciales de v1 permanecen aislados y no se reutilizan.

La cohorte principal se distribuye en cuatro shards concurrentes, balanceados y no solapados; la cohorte de estrés, al contener sólo dos unidades, usa dos. Cada dataset conserva un \textttbreak{protocol\_index} canónico, de modo que dividir la campaña no modifica la seed usada para construir historias y queries. Cada shard mantiene juntos todos los modelos y seeds de sus datasets para reutilizar la preparación y las cachés, escribe logs y sentinels propios, y sólo se publica un consolidado después de validar la unión exacta sin duplicados ni faltantes. La lectura de Parquet se omite al reanudar un dataset ya completo y compatible. Durante evaluación se reutilizan transferencias CPU--GPU, se vectoriza la construcción de métricas y se preindexan diagnósticos por sensor. La historia determinista submuestreada puede cachearse; los targets nunca se cachean y las queries de entrenamiento continúan cambiando por época.

El grado de concurrencia se eligió mediante probes de ingeniería ajenos al ranking científico. Sobre el mismo conjunto de cuatro trabajos, cuatro procesos tardaron 35.928 segundos frente a 57.278 con dos. Para ocho trabajos, dos tandas de cuatro tardaron 69.566 segundos y ocho procesos simultáneos 71.218; por ello se descartó aumentar la concurrencia a ocho. También se descartaron batches mayores y workers adicionales: el primero alteraba la dinámica de optimización y el segundo no redujo el tiempo total en Windows. Estas mediciones justifican la configuración de ejecución, pero no se interpretan como resultados de precisión del modelo.
...
El subcomando \textttbreak{all} ejecuta preflight, shards físicos, controles de identificabilidad y reporte. Ante una interrupción, repetir el mismo comando reanuda únicamente artefactos completos con fingerprint compatible.
